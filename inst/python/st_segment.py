import skimage.color, skimage.io, skimage.measure
...
def _calculate_morph_stats(tile_path):
    ...
    channel = htk.preprocessing.color_deconvolution.find_stain_index(stain_color_map[stains[0]], w_est)
    ...
    im_fgnd_mask = sp.ndimage.morphology.binary_fill_holes(im_nuclei_stain < 0.8 * thresh)
    ...
    min_nucleus_area = 60
    im_nuclei_seg_mask = htk.segmentation.label.area_open(labels, min_nucleus_area).astype(np.int)
