...
if(method == "SingleR"){
 ...
 if(cluster == FALSE){
   singler1<-SingleR::SingleR(test= test_for_singler , ref = ref ,labels = label)
...
names(clusters)<-rownames(x@meta.data)
 singler2<-SingleR::SingleR(test= test_for_singler,ref = ref,labels = label,clusters=clusters)
